...
namespace Serial_ESCPOS
{
    public partial class Form1 : Form
    {
        ...
        {
            this.textBox1.Text = "实时打印机请求，从可恢复的错误中立即恢复，并从之前发生错误的行继续打印!\r\n";
            
            // DLE ENQ n = 1~2
            byte[] dleEnq = new byte[] { 0x10, 0x05, 0x01 };
            ipWrite(dleEnq, 0, dleEnq.Length);

            ipWrite("实时打印机请求，从可恢复的错误中立即恢复，并从之前发生错误的行继续打印!\n\n");
        }

        private void button2_Click(object sender, EventArgs e)
        {
            this.textBox1.Text = "实时打印机请求，先删除接收与打印缓冲，再从可恢复的错误中恢复!\r\n";

            // DLE ENQ n = 1~2
            byte[] dleEnq = new byte[] { 0x10, 0x05, 0x02 };
            ipWrite(dleEnq, 0, dleEnq.Length);

            ipWrite("实时打印机请求，先删除接收与打印缓冲，再从可恢复的错误中恢复!\n\n");
        }
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            // GS ( D
            // 默认允许产生脉冲的操作，并禁止电源关闭流程；
            // 对应影响DLE DC4 fn=1 和 2
        ...
        {
            this.textBox1.Text = "禁止纸将尽检测\r\n";

            // ESC c 4
            byte[] closePpaperNearEnd = new byte[] { 0x1B, 0x63, 0x34, 0x00 };
            ipWrite(closePpaperNearEnd, 0, closePpaperNearEnd.Length);
        ...
        {
            this.textBox1.Text = "启用纸将尽检测\r\n";

            // ESC c 4
            byte[] openPpaperNearEnd = new byte[] { 0x1B, 0x63, 0x34, 0x03 };
            ipWrite(openPpaperNearEnd, 0, openPpaperNearEnd.Length);
