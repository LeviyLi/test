...
namespace Serial_ESCPOS
{
    public partial class Form1 : Form
    {
        ...
        {
            // GS (/8 L, Transmit the remaining capacity of the download graphics memory
            // <Function 52>
            byte[] gsNV = new byte[] { 0x1D, 0x28, 0x4C, 0x02, 0x00, 0x30, 0x04 };
            ipWrite(gsNV, 0, gsNV.Length);

            textBox1.Text = "查询图形缓冲剩余容量!\r\n";
            ipWrite("\n2.1 <GS ( L> 查询图形缓冲剩余容量!\n\n");
        }

        private void button7_Click(object sender, EventArgs e)
        {
            // GS (/8 L, Transmit the key code list for defined download graphics
            // <Function 80>
            byte[] gsNV = new byte[] { 0x1D, 0x28, 0x4C, 0x04, 0x00, 0x30, 0x50, 0x4B, 0x43 };
            ipWrite(gsNV, 0, gsNV.Length);

            textBox1.Text = "读取图形缓冲中的所有索引!\r\n";
            ipWrite("\n2.2 <GS ( L> 读取图形缓冲中的所有索引!\n\n");
        }

        private void button8_Click(object sender, EventArgs e)
        {
            // GS (/8 L, Delete all download graphics data
            // <Function 81>
            byte[] gsNV = new byte[] { 0x1D, 0x28, 0x4C, 0x05, 0x00, 0x30, 0x51, 0x43, 0x4C, 0x52 };
            ipWrite(gsNV, 0, gsNV.Length);

            textBox1.Text = "删除图形缓冲中的所有索引对应BMP!\r\n";
            ipWrite("\n2.3 <GS ( L> 删除图形缓冲中的所有索引对应BMP!\n\n");
        }

        private void button9_Click(object sender, EventArgs e)
        {
            ipWrite("\n\n1.1 <GS D> 选择Windows BMP文件与指定索引绑定，并存放到图形缓冲中!\n");
            ipWrite("\n\n索引范围：\n  32<= key1 <=126\n  32<= key1 <=126\n\n");
            textBox1.Text = "<GS D>将Windows BMP文件与指定索引绑定，并存放到图形缓冲中!\r\n";
            ...
            else
            {
                ...
                // GS D <Function 83>; Define Windows BMP download file
                // Key code = (0x20, 0x20)
                byte[] gsBmp = new byte[] { 0x1D, 0x44, 0x30, 0x53, 0x30, 0x20, 0x20, 0x30, 0x31 };
                ...
                ipWrite("\n\n完成BMP存到图形缓冲，并与索引<" + comboBox4.Text + ">绑定!\n\n");
                fs.Close();
                fs.Dispose();
            }
        }
        ...
        {
            textBox1.Text = "打印图形缓冲中索引<" + comboBox4.Text + ">对应的BMP!\r\n";
            ipWrite("\n1.2 <GS ( L>打印图形缓冲中索引<" + comboBox4.Text + ">对应的BMP!\n\n");
            
            // GS (/8 L, Print the specified NV graphics data
            // <Function 85>
            byte[] gsBmp = new byte[] { 0x1D, 0x28, 0x4C, 0x06, 0x00, 
                                        0x30, 0x55, 0x20, 0x20, 0x01, 0x01 };
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            byte[] gsBmp = new byte[] { 0x1D, 0x28, 0x4C, 0x04, 0x00, 0x30, 0x52, 0x20, 0x20 };
            ...
            textBox1.Text = "删除图形缓冲中索引<" + comboBox4.Text + ">对应的BMP!\r\n";
            ipWrite("\n1.3 <GS ( L> 删除图形缓冲中索引<" + comboBox4.Text + ">对应的BMP!\n\n");
        }
